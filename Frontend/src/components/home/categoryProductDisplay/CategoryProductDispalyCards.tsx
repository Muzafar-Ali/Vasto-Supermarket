      ...
      <div className='flex items-center justify-between'>
        {products.length > 0 ? (
          products.map((product) => (
            <div key={product._id} className='border p-4 grid gap-3 max-w-52 lg:min-w-52 rounded'>
              {/* image */}
              <div className='min-h-20 max-h-32 bg-blue-50 rounded'>
                <Image src={product?.imageUrl[0]} alt={product.name} width={1000} height={1000} className='h-full w-full object-scale-down hover:scale-125 transition-all duration-200'/>
              </div>
              ...
              <div className='font-medium text-ellipsis line-clamp-2'>{product.name}</div>
              <div className='w-fit'>{product.unit}</div>
              <div className='flex items-center justify-between'>
