  ...
  return (
    <Link href={`/product/${product.slug}/${product._id}`} className="block w-full">
      <div className='border p-2 lg:p-4 grid gap-1 lg:gap-3 w-full h-full rounded-lg cursor-pointer shadow-md bg-white'>
        {/* image */}
        <figure className='min-h-20 w-full max-h-24 lg:max-h-32 rounded overflow-hidden'>
          <Image 
            src={product?.imageUrl[0]} 
            alt={product.name} 
            width={1000} 
            height={1000} 
            className='h-full w-full object-scale-down lg:scale-125 transition-all duration-200'
          />
        </figure>
        ...
        <div className='rounded text-xs w-fit py-[1px] px-2 text-green-600 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-2 lg:gap-5 min-h-[45px]'>
         ...
         {/* Product Name */}
        <h2 className='font-medium text-sm lg:text-base text-ellipsis line-clamp-2 min-h-[40px]'>{product.name} </h2>
        {/* Product Unit */}
